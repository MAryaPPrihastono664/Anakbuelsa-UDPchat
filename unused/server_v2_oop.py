...
class server_fui:
    ...
    def sent_back(self):
        while self.is_running:
            while not self.messages.empty():
                ...
                match content[0]:
                    case "!":
                        a = self.send_error(addr,message)
                    case "0":
                        a = self.commandHelp(addr)
                    case "1":
                        a = self.commandRegister(addr,content)
                    case "2":
                        a = self.commandLogin(addr,content)
                    case "3":
                        a = self.commandLogout(addr,content)
                    case "4":
                        a = self.commandRemoveAcc(addr,content)
                    case "5":
                        a = self.commandCreateChat(addr,content)
                    case "6":
                        a = self.commandJoinChat(addr,content)
                    case "7":
                        a = self.commandLeaveChat(addr,content)
                    case "8":
                        a = self.commandEcho(addr,content)
                    case "9":
                        a = self.commandSendToChat(addr,content)
                # if __name__ == "server_fui":
                    # ui.print_info(decodedMessage)
                    # ui.print_status(a)
    ...
    def start(self):
        ...
        tReceive.start()
        tSendToClient.start()
        # ui.scroll_screen.insert(tk.END, "server start\n")
    ...
    def stop(self):
        ...
        # ui.scroll_screen.insert(tk.END, "Server stopped\n")
